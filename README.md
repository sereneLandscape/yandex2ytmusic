...
Перенос понравившихся треков и подкастов из Yandex.Музыка в YouTube Music.
...
- **Раздельный экспорт/импорт** — можно сначала экспортировать треки и подкасты из Яндекса, а потом отдельно импортировать в YouTube (решает проблему таймаута сессии при большом количестве треков)
...
- **Выбор режима импорта** — быстрый параллельный режим или режим с сохранением порядка треков и подкастов
...
Треки и подкасты сохранятся в файл `tracks.json`. Многопоточная обработка ускоряет процесс в ~5 раз.
...
Программа загрузит треки и подкасты из `tracks.json` и добавит их в лайки YouTube Music.
